...
function add(){var $add=$(exports.config.add_id),$control_container=$(exports.config.control_container_id);if(!$add[0])return false;$add.on('click',function(){var $tpl=$(exports.config.tpl.replace(/\%placeholder\%/ig,get_random_int(100,999)));del($tpl);$control_container.before($tpl);$tpl.find('input').eq(0).focus();});}
function del($tpl){$tpl.find('.delete').on('click',function(){$($(this).data('target')).css('background','#d54e21').fadeOut('slow',function(){$(this).remove();})})}
