...
Description: A simple and easy way to fix your gravatar can not be show in China. Replace by v2ex cdn. 
Author: INN STUDIO
...
if(!class_exists('theme_gravatar_fix')){
	...
	class theme_gravatar_fix{
		public static function get_gravatar($avatar) {
			...
			$avatar = str_ireplace(
				array('d=','/avatar'),
				array('nodefault=','/gravatar'),
				$avatar
			);
			return preg_replace('/(\d)?(secure)?([a-z]{0,2})\.gravatar\.com/i', 'cdn.v2ex.com', $avatar);
		}
	}
}