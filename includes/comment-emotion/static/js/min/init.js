...
function insert(){function insert_content(){cache.$comment.focus();var caret_pos=cache.$comment.selectionStart,old_val=cache.$comment.value;cache.$pop[cache.active_pop_i].style.display='none';cache.$comment.value=old_val.substring(0,caret_pos)+' '+this.getAttribute('data-content')+' '+old_val.substring(caret_pos);cache.pop_hide[cache.active_pop_i]=true;cache.$pop[cache.active_pop_i].style.display='none';}
