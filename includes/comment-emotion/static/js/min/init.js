...
function insert(){function insert_content(){setTimeout(function(){cache.$comment.focus();},1);var caret_pos=cache.$comment.selectionStart,old_val=cache.$comment.value;cache.$pop[cache.active_pop_i].style.display='none';cache.$comment.value=old_val.substring(0,caret_pos)+' '+this.getAttribute('data-content')+' '+old_val.substring(caret_pos);cache.pop_hide[cache.active_pop_i]=true;cache.$pop[cache.active_pop_i].style.display='none';}
for(var i=0,len=cache.$emotion_faces.length;i<len;i++){cache.$emotion_faces[i].addEventListener('click',insert_content);}}
function pop(){function hide_pop(e){e.preventDefault();if(cache.active_pop_i!==false)
...
if(!cache.replaced[cache.active_pop_i]){cache.replaced[cache.active_pop_i]=true;var $imgs=cache.$pop[cache.active_pop_i].querySelectorAll('img');for(var i=0,len=$imgs.length;i<len;i++){$imgs[i].src=$imgs[i].getAttribute('data-src');$imgs[i].removeAttribute('data-src');}}}
