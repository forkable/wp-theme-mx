...
done(data);}else{fail();}};xhr.onerror=function(){tools.ajax_loading_tip('error',config.lang.E01);cache.$dialog_new_uid.select();};function done(data){if(data.status==='success'){config.userdata[uid]={avatar:data.avatar,name:data.name};tools.ajax_loading_tip(data.status,data.msg,3);create_tab(uid);create_close(uid);if(!data.histories){create_dialog(uid,get_tpl_msg(uid,config.lang.M05.replace('%name%',config.userdata[uid].name)));}else{create_dialog(uid,get_histories(data.histories));}
...
function get_tpl_msg(uid,msg){var d=new Date(),d=date_format(d,'yyyy/MM/dd hh:mm:ss'),sender=uid==='me'?'me':'sender';return'<section class="pm-dialog-'+sender+'">'+'<div class="pm-dialog-bg">'+'<h4>'+'<span class="name">'+config.userdata[uid].name+'</span> '+'<span class="date"> '+d+' </span>'+'</h4>'+'<div class="media-content">'+msg+'</div>'+'</div>'+'</section>';}
...
done(data);}else{fail();}};xhr.onerror=fail;function done(data){if(data&&data.status==='success'){var author_uid=data.pm.pm_author,receiver_uid=data.pm.pm_receiver;cache.timestamp=data.timestamp;if(author_uid==config.my_uid&&cache.$dialogs[receiver_uid]){insert_dialog_msg('me',data.pm.pm_content);focus_clear_input(receiver_uid);}else{if(!config.userdata[author_uid]){config.userdata[author_uid]={name:data.pm.pm_author_name,avatar:data.pm.pm_author_avatar,};}
