...
urls+='&special['+validates[i].getAttribute('data-ajax-type')+']='+validates[i].value;}
$this.style.display='none';tip($this,'loading',config.lang.M00001);var xhr=new XMLHttpRequest();xhr.open('GET',config.process_url+urls);xhr.onload=function(){if(xhr.status>=200&&xhr.status<400){var data;try{data=JSON.parse(xhr.responseText);}catch(e){}
...
$this.style.display='';};xhr.onerror=function(){tip($this,'error',config.lang.E00001);$this.style.display='';};xhr.send();return false;}
function tip(el,t,s){if(!el)
return false;var $tip=document.getElementById(el.getAttribute('data-target'));$tip.innerHTML=tools.status_tip(t,s);$tip.style.display='block';}
