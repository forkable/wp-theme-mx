...
define(function(require,exports,module){'use strict';var js_request=require('theme-cache-request'),tools=require('modules/tools');exports.config={iden:'theme_comment_ajax',process_url:'',pagi_process_url:'',post_id:'',lang:{M00001:'Loading, please wait...',M00002:'Commented successfully, thank you!',M00003:'Previous',M00004:'Next',M00005:'{n} page',E00001:'Sorry, some server error occurred, the operation can not be completed, please try again later.'}};var cache={},config=exports.config;exports.init=function(){tools.ready(function(){exports.count.set();cache.$comment_list_container=I('comment-list-'+config.post_id);if(!cache.$comment_list_container)
return false;window.addComment=addComment;exports.list.init();var pagi=new exports.pagination();pagi.lang.loading=config.lang.M00001;pagi.lang.error=config.lang.E00001;pagi.lang.prev=config.lang.M00003;pagi.lang.next=config.lang.M00004;pagi.lang.midd=config.lang.M00005;pagi.pages=js_request[config.iden].pages;pagi.cpage=js_request[config.iden].cpage;pagi.url_format=config.pagi_process_url;pagi.init();var rsp=new exports.respond();rsp.init();});};exports.count={set:function(n){cache.$count=document.getElementById('comment-number-'+config.post_id);if(!cache.$count)
...
return false;cache.$comment_list_container.innerHTML=js_request[config.iden].comments;},get:function(){var _list=this,xhr=new XMLHttpRequest(),param=tools.param({'type':'get-comments','post-id':config.post_id});xhr.open('GET',config.pagi_process_url+'&'+param);xhr.send();xhr.onload=function(){if(xhr.status>=200&&xhr.status<400){var data;try{data=JSON.parse(xhr.responseText);}catch(e){}
if(data&&data.status){_list.done(data);}else{_list.fail(xhr.responseText);}
...
return false;_cache.$container.appendChild(create());set_cache(_that.cpage,cache.$comment_list_container.innerHTML);}
function set_cache(cpage,comments){if(!_cache.comments)
...
function create_prev(){var prev_class=_that.cpage<=1?'disabled':'',attrs={'class':'prev btn btn-default '+prev_class,'href':'javascript:;'};_cache.$prev=document.createElement('a');for(var k in attrs){_cache.$prev.setAttribute(k,attrs[k]);}
...
exports.respond=function(){var _cache={},_config={logged:js_request[config.iden].logged,registration:js_request[config.iden].registration,prefix_comment_body_id:'comment-body-'},_that=this;this.init=function(){goto_click();fm_init();};function fm_init(){_cache.$respond=I('respond');_cache.$fm=I('commentform');_cache.$must_logged=I('respond-must-login');_cache.$loading_ready=I('respond-loading-ready');_cache.$avatar=I('respond-avatar');_cache.$area_visitor=I('area-respond-visitor');_cache.$comment_parent=I('comment_parent');_cache.$comment_ta=I('comment-form-comment');_cache.$submit_btn=_cache.$fm.querySelector('.submit');if(!_cache.$respond||!_cache.$fm)
return false;if(_cache.$loading_ready)
...
tools.ajax_loading_tip('loading',config.lang.M00001);_cache.$submit_btn.setAttribute('disabled',true);ajax();return false;}
...
if(data&&data.status==='success'){var $new_comment=tools.parseHTML(data.comment)[0];if(_cache.$comment_parent.value!=0){var $parent_comment_body=I(_config.prefix_comment_body_id+_cache.$comment_parent.value);$parent_comment_body.insertAdjacentHTML('afterend','<ul class="children">'+$new_comment.innerHTML+'</ul>');addComment.cancelMove();}else{cache.$comment_list_container.appendChild($new_comment);}
_cache.$comment_ta.value='';tools.ajax_loading_tip(data.status,data.msg,3);}else if(data&&data.status==='error'){tools.ajax_loading_tip(data.status,data.msg);}else{tools.ajax_loading_tip('error',config.lang.E00001);}}else{tools.ajax_loading_tip('error',config.lang.E00001);}
_cache.$submit_btn.removeAttribute('disabled');};xhr.onerror=function(){tools.ajax_loading_tip('error',config.lang.E00001);_cache.$submit_btn.removeAttribute('disabled');}}
function goto_click(){_cache.$goto=I('goto-comment');_cache.$comment=I('comment-form-comment');if(!_cache.$goto||!_cache.$comment)
return;_cache.$goto.style.display='block';_cache.$goto.onclick=function(){setTimeout(function(){_cache.$comment.focus();},300);}}}
...
t.cache.$comment.focus();return false;}};function I(e){return document.getElementById(e);}});