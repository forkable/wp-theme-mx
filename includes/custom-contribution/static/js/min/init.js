...
cache.file_index++;if(data&&data.status==='success'){append_tpl(data);var editor_content=send_content(data.full.url,data[config.default_size].url);if(cache.$split_number.value>=1&&cache.file_index>1){if(cache.file_index%cache.$split_number.value==0)
...
var $insert_btn=$tpl.querySelectorAll('.ctb-insert-btn'),send_content_helper=function(){var editor_content=send_content(args.full.url,args[this.getAttribute('data-size')].url);send_to_editor(editor_content);};for(var i=0,len=$insert_btn.length;i<len;i++){$insert_btn[i].addEventListener('click',send_content_helper,false);}
return $tpl;}
