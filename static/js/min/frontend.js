...
define(function(require,exports,module){'use strict';require.async(['modules/lazyload','modules/bootstrap-without-jq'],function(_a,_b){});var tools=require('modules/tools');exports.config={is_home:false};exports.init=function(){tools.ready(function(){exports.hide_no_js();exports.search();exports.page_nagi();});}
exports.page_nagi=function(){var $post=document.querySelector('.singluar-post'),$nagi=document.querySelector('.page-pagination'),post_top,max_bottom,is_hide=false;if(!$post||!$nagi)
return;reset_nagi_style();window.addEventListener('resize',function(){reset_nagi_style()});$nagi.style.display='block';function reset_nagi_style(){post_top=getElementTop($post);max_bottom=post_top+$post.querySelector('.panel-body').clientHeight;$nagi.style.left=getElementLeft($post)+'px';$nagi.style.width=$post.clientWidth+'px';}
